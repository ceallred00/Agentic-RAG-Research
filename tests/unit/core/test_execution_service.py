...
class TestExecutionService:
    ...
    class TestGetGeminiClient:
        ...
        def test_happy_path(
            self,
            gemini_api_key_env,
            instance_agent_config_execution_service,
            mock_gemini_client,
        ):
            ...
            mock_gemini_client.assert_called_once_with(
                model="gemini-3-small", temperature=0.7, api_key=gemini_api_key_env
            )
    ...
    class TestGetEmbeddingClient:
        ...
        def test_gemini_embedding_client_creation_failure(
            self,
            instance_execution_service,
            mock_gemini_dense_embedding_client,
            gemini_api_key_env,
            ...
            with pytest.raises(Exception) as excinfo:
                instance_execution_service.get_embedding_client(
                    model_name="gemini-embedding-001", task_type="RETRIEVAL_QUERY"
                )
    ...
    class TestGetPineconeClient:
        ...
        def test_get_pinecone_client_happy_path(
            self, instance_execution_service, mock_pinecone_client, pinecone_api_key_env
        ):
        ...
        def test_pinecone_client_creation_failure(
            self, instance_execution_service, mock_pinecone_client, pinecone_api_key_env
        ):
