...
class TestConfluenceContentExtractor:
    class TestExtract:
        def test_extract_happy_path(self, confluence_html_extractor, sample_url, sample_space_key):
            ...
            result = confluence_html_extractor.extract(
                raw_html=raw_html, base_url=sample_url, space_key=sample_space_key
            )
        ...
        def test_extract_returns_empty_string_on_empty_input(
            self, confluence_html_extractor, sample_url, sample_space_key
        ):
            """
            ...
            with patch("knowledge_base.ingestion.confluence_content_extractor.BeautifulSoup") as mock_bs:
                result = confluence_html_extractor.extract(
                    raw_html=empty_html, base_url=sample_url, space_key=sample_space_key
                )
        ...
        def test_extract_removes_empty_paragraphs_and_whitespaces(
            self, confluence_html_extractor, sample_url, sample_space_key
        ):
            """
