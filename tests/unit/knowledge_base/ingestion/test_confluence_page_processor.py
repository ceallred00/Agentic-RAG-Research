...
class TestConfluencePageProcessor:
    class TestProcessPage:
        ...
        def test_process_page_happy_path(
            self,
            ...
            expected_content, expected_page_title = self._expected_values(
            ...
            confluence_processor.file_saver.save_markdown_file.assert_called_once_with(
                expected_content, expected_page_title
            )
            assert result is None

        def test_process_page_skips_empty_content(
            self,
            sample_confluence_page_json,
            confluence_processor,
            caplog,
        ...
        def test_process_page_missing_metadata(
            self,
            confluence_processor,
            sample_confluence_page_json,
            sample_url,
        ...
        @pytest.mark.parametrize(
            "ancestors, case_description",
            [
                ([], "Root Page (No Ancestors)"),
                (
                    [
                        {"id": "12345", "title": "Grandparent"},
                        {"id": "6789", "title": "Parent"},
                    ],
                    "Nested Page (Multiple Ancestors)",
                ),
            ],
        )
        def test_process_page_ancestor_handling(
            self,
            confluence_processor,
            sample_url,
            mock_cleaned_html,
        ...
        def test_process_page_cleans_artifacts(
            self,
            confluence_processor,
            sample_confluence_page_json,
            sample_url,
            ...
            confluence_processor.file_saver.save_markdown_file.assert_called_once_with(
                expected_content, "Advising Syllabus"
            )
